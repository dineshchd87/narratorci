...
if (count($_FILES)) {
	...
	$uploadDir = '../../../scripts-upload/';
	
	$fileFinal = $uploadDir . $fileName;
	
	$upload = move_uploaded_file($source, $fileFinal);
	chmod($fileFinal,0755);
}
...
if ($error) {
	...
	$return = array(
		'status' => '1',
		'name' => isset($fileName)? $fileName : $_FILES['Filedata']['name']
	);
